...
object hello {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //val select = spark.sql("select order_seq,auth_id,org_id,card_no from order_info limit 20 ")
    //select.show()

    val org_info = utils.get_tables("org_info")
    //org_info.printSchema()
    org_info.createOrReplaceTempView("org_info")
    val join = spark.sql("select a.auth_id from order_info a left join org_info b on a.org_id = b.org_id limit 5")
    join.show()*/
